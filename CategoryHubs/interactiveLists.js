...
function createAnswerForm( button ) {
	...
	var url = wgServer + wgScriptPath + '/api.php?format=json&action=query&prop=revisions&rvprop=content&titles=' + this_title;
	jQuery.getJSON( url, '', function( j ) {
		var existing_content = '';
		if ( j.query.pages ) {
			for ( var page in j.query.pages ) {
	...
	$( button ).parents( 'li' ).find( '.cathub-button-save' ).click( function() {
		...
		jQuery.getJSON( url, '', function( editPage ) {
			if ( editPage.query.pages ) {
				...
				url = wgServer + wgScriptPath + '/api.php?format=json&token=' +
					encodeURIComponent( token ) + '&action=edit&title=' +
					this_title + '&text=' + encodeURIComponent( add_answer.value );
					...
					if ( j.error ) {
						...
						jQuery.getJSON( url, '', function( j ) {
							...
							jQuery.getJSON( url, '', function( j ) {
								...
								var answerCode = '';
								answerCode += '<span class="cathub-answer-heading">' +
									wgCatHubAnswerHeadingMsg + "</span><br />\n";
								...
								if ( $( button ).parents( 'li' ).find( '.cathub-actual-answer' ).size() > 0 ) {
									...
									// To make the progress more visible, make
									// the div bg green and purge the memcache
									// for percent and for the lists.
