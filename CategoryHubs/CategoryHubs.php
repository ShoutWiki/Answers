<?php
/**
 * This extension seeks to turn category pages into a hub of activity centered
 * around that category.
 * The goal is to make it easy for contributors to use this page to quickly see
 * what's going on and act on the answered/unanswered questions.
 *
...
function categoryHubAdditionalScripts( &$out, &$sk ) {
	global $wgScriptPath;
	$out->addExtensionStyle( $wgScriptPath . '/extensions/CategoryHubs/CategoryHubs.css' );
	...
	$out->addScriptFile( $wgScriptPath . '/extensions/CategoryHubs/interactiveLists.js' );
	return true;
}
 ...
 */
function categoryHubJsGlobalVariables( &$vars, OutputPage $out ) {
	global $wgStylePath;
	...
	$vars['wgPurgeCategoryHubsCacheUrl'] = $out->getTitle()->getFullURL( 'action=purge&exitwithoutcontent=1' );
...
function categoryHubCheckForMagicWords( &$parser, &$text, &$strip_state ) {
	...
	} elseif ( isset( $wgCatHub_useDefaultView ) && $wgCatHub_useDefaultView ) {
		$mw = MagicWord::get( CATHUB_NORICHCATEGORY );
...
function categoryHubBeforeArticleText( &$flexibleCategoryPage ) {
	global $wgCatHub_useDefaultView, $wgRequest;
	$wgCatHub_useDefaultView = $wgRequest->getBool( 'useoldcats', $wgCatHub_useDefaultView );

	// Since this is executed before the parser gets called (and thus the
	// hooks), must also check for the magic word here
	// (unless it was already found elsewhere).
	if ( ( !isset( $wgCatHub_useDefaultView ) ) || ( !$wgCatHub_useDefaultView ) ) {
		$mw = MagicWord::get( CATHUB_NORICHCATEGORY );
		// match does not return bool as documented. Fix is committed to MediaWiki SVN.
		$wgCatHub_useDefaultView = ( 0 < $mw->match( $flexibleCategoryPage->getRawText() ) );
	}
 ...
 * We will use it to determine ahead of time if the page has rich categories
 * enabled or not.
 *
...
function categoryHubInitViewer( &$flexibleCategoryViewer ) {
	...
	// Since this and some other functions that need this info are executed
	// before the parser gets called (and thus the hooks for checking the magic words),
	// we check for the magic word here (unless it was already found elsewhere).
	// This is used in categoryHubDoCategoryQuery and some of the
	...
	if ( ( !isset( $wgCatHub_useDefaultView ) ) || ( !$wgCatHub_useDefaultView ) ) {
		...
		// match does not return bool as documented. Fix is committed to MediaWiki SVN.
		$wgCatHub_useDefaultView = ( $article && ( 0 < $mw->match( $article->getRawText() ) ) );
	}
...
function categoryHubDoCategoryQuery( &$flexibleCategoryViewer ) {
	...
	// Order by "page_touched" instead of alphabetically.
	// Keep in mind that if pagination starts being used, that
	// it will have to be modified to include the page_touched values in the 'from' and 'until'
	// parameters instead of the page names (will require changes in which
	// parameter is used when because the order is DESC by default here and ascending by default normally).
	if ( !$wgCatHub_useDefaultView ) {
		// If we haven't passed the limit, store the entire article for the answer so that
		// extensions (such as CategoryHubs) can display in-depth data.
...
function categoryHubTitleBar( &$catView, &$r ) {
	...
	if ( $logoSrc != '' ) {
		$r .= '<img src="' . $logoSrc . '" height="78" alt="" class="cathub-title-bar-wikilogo" />';
	}
	...
	//$r .= " <img id='cathub-notify-me' src='$wgScriptPath/extensions/CategoryHubs/notify.png' width='114' height='74' style='float:right;padding:5px;'/>";
	//$r .= '</div>';
	...
	$answeredCategory = CategoryHub::getAnsweredCategory();
	$unAnsweredCategory = CategoryHub::getUnAnsweredCategory();
	list( $percentAnswered, $countAnswered, $countUnAnswered ) =
		$categoryEdits->getPercentInCats( $answeredCategory, $unAnsweredCategory );
	$allQuestions = $countAnswered + $countUnAnswered;
	if ( $percentAnswered <= 0 ) {
		$percentAnswered = 0;
		$r .= "<div class='cathub-progbar-unanswered' style='width:$PROG_BAR_WIDTH'>" .
			wfMsgExt( 'cathub-progbar-none-done', array() ) . "</div>\n";
	} elseif ( $percentAnswered >= 100 ) {
		if ( empty( $countUnAnswered ) ) {
			$percentAnswered = 100;
			$r .= "<div class='cathub-progbar-answered' style='width:$PROG_BAR_WIDTH' title=''>" .
				wfMsgExt( 'cathub-progbar-all-done', array() ) . "</div>\n";
		} else {
			// some unanswered questions
			$r .= "<div class='cathub-progbar-answered' style='width:$PROG_BAR_WIDTH' title=''>" .
				wfMsgExt( 'cathub-progbar-allmost-done', 'parsemag', $countUnAnswered ) . "</div>\n";
		}
	} else {
		// TODO: EXTRACT THIS TO A FUNCTION WHICH WILL MAKE A
		// BANDWIDTH-EFFICIENT PROGRESS BAR FOR ANY USE
		// (IF POSSIBLE TO DO CLEANLY... MIGHT HAVE TO REQUIRE IT TO BE ANSWERS-SPECIFIC).
		...
		// Heuristic to figure out which side to put the text on (prefering to put it on the left
		// whenever possible since it is more intuitive to see the percent done rather than not done).
		// Since users have various font-sizes, this is meant to give a sizable leeway.
		$aText = $uText = '&nbsp;';
		...
		if ( $aPercent < $MIN_PERCENT_TO_SHOW_TEXT ) { // if possible, be less confusing by leaving the number on the left.
			$aText = '&nbsp;';
			$uText = '&nbsp;';
		} elseif ( $uPercent < $MIN_PERCENT_TO_SHOW_TEXT ) {
	...
	$r .= '<div class="cathub-progbar-label cathub-progbar-label-left">' .
		wfMsgExt( 'cathub-progbar-label-answered', array() ) . '</div>';
	$r .= '<div class="cathub-progbar-label cathub-progbar-label-right">' .
		wfMsgExt( 'cathub-progbar-label-unanswered', array() ) . '</div>';
...
function categoryHubGetLogo( $cat_name ) {
	...
	if ( empty( $cityId ) ) {
		$cityId = WikiFactory::DomainToId( str_replace( ' ', '', $cat_name ) . '.wikia.com' ); //Wikia stuff - and WikiFactory
	}
	if ( empty( $cityId ) ) {
		return '';
	}
...
function categoryHubTopContributors( &$catView, &$r ) {
	...
	$r .= categoryHubContributorsToHtml(
		$categoryEdits->getContribs( $show_staff, $NUM_CONTRIBS_PER_SECTION )
	);
	$r .= "</div>\n";
	...
	$r .= categoryHubContributorsToHtml(
		$categoryEdits->getXDayContribs(
			CATHUB_RECENT_CONTRIBS_LOOKBACK_DAYS,
			$show_staff,
			$NUM_CONTRIBS_PER_SECTION
		)
	);
	$r .= "</div>\n";
...
/**
 * Given an array of user ids mapped to and how many edits they've had in the valid lookback period
 * for this category, (as returned from CategoryEdits::getContribs or CategoryEdits::getXDayContribs, etc.),
 * returns the HTML for an ordered list of the users.
 */
...
function categoryHubOtherSection( &$catView, &$r ) {
	...
	if ( !$wgCatHub_useDefaultView ) {
		...
		// The tabs.
		$r .= "<ul>\n";
		$r .= '<li><a href="#cathub-tab-unanswered" id="cathub-tablink-unanswered"><span>' .
			wfMsgExt( 'unanswered_category', array() ) . "</span></a></li>\n";
		$r .= '<li><a href="#cathub-tab-answered" id="cathub-tablink-answered"><span>' .
			wfMsgExt( 'answered_category', array() ) . "</span></a></li>\n";
		$r .= "</ul>\n";
		...
		if ( empty( $catView->answerArticles[$UN_CLASS] ) || count( $catView->answerArticles[$UN_CLASS] ) == 0 ) {
			...
			foreach ( $catView->answerArticles[$UN_CLASS] as $qArticle ) {
				if ( is_object( $qArticle ) ) {
					...
					$r .= "<span class=\"$UN_CLASS cathub-article-link\">" .
						Linker::linkKnown( $title, $title->getPrefixedText() . '?' ) . '</span>';
		...
		if ( $numReturned_u > 0 ) {
			$r .= categoryHubPagination( $catView->title, $wgCategoryHubArticleLimitPerTab, $offset_u, $numReturned_u, $U_SUFFIX );
		}
		$r .= "&nsbp;</div>\n";
		/*if ( $wgUser->isAnon() ) {
		...
		if ( empty( $catView->answerArticles[$ANS_CLASS]) || count( $catView->answerArticles[$ANS_CLASS] ) == 0 ) {
			...
			foreach ( $catView->answerArticles[$ANS_CLASS] as $qArticle ) {
				if ( is_object( $qArticle ) ) {
					...
					// Question & attribution for last edit.
					$title = $qArticle->getTitle();
					$r .= "<span class=\"$ANS_CLASS cathub-article-link\">" .
						Linker::linkKnown( $title, $title->getPrefixedText() . '?' ) . '</span>';
					...
					$r .= '<span class="cathub-answer-heading">' .
						wfMsgExt( 'cathub-answer-heading', array() ) . "</span><br />\n";
		...
		if ( $numReturned_a > 0 ) {
			$r .= categoryHubPagination( $catView->title, $wgCategoryHubArticleLimitPerTab, $offset_a, $numReturned_a, $A_SUFFIX );
		}
		$r .= "&nbsp;</div>\n";
 ...
 *
 * @param Title $title Title object to link to
 ...
 */
function categoryHubPagination( $title, $limit, $offset, $numReturned, $suffix ) {
	global $wgTitle;
	...
	if ( $offset > 0 ) {
		$html .= Linker::link(
			$wgTitle,
			wfMsgExt( 'cathub-prev', array() ),
			array( 'rel' => 'previous' ),
			array( "offset$suffix" => ( $offset - $limit ) ),
			array( 'known' )
		);
	} else {
		$html .= '&nbsp;';
 ...
 *
 * @param $qArticle Object: instance of Article
 * @param $answered Boolean: If true, then the text will say "answered by" instead of "asked by".
 */
function categoryHubGetAttributionByArticle( $qArticle, $answered = false ) {
	...
	$lastUpdate = wfTimeFormatAgo( $timestamp ); // @todo FIXME
	if ( $userId > 0 ) {
