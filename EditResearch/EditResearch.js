/**
 * JavaScript for EditResearch extension
 */
		...
		if( data.query ) {
			...
			if( search_html ) {
				search_html += '<div id="research-search-nav">' +
					( ( research_page > 0 ) ? '<a href=javascript:research_paginate(-1)>' + wgPrevMsg + '</a>' : '' ) +
					' <a href=javascript:research_paginate(1)>' + wgNextMsg + '</a></div>';
			} else {
...
function research_wikipedia_article( node ) {
	...
	url = wikipedia_server + '/w/api.php?action=parse&page=' + encodeURIComponent( article ) + '&format=json&callback=?';
